...
test('throws on code syntaxError', async t => {
  await t.throwsAsync(async () => {
    const [err, data] = await safeAwait(promiseWithSyntaxError())
  }, {instanceOf: ReferenceError, message: 'madeUpThing is not defined'})
})

test('throws on code syntaxError in try/catch', async t => {
  try {
    const [err, data] = await safeAwait(promiseWithSyntaxError())
  } catch (e) {
